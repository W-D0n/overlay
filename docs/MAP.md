...
| S8 | Moteur de scène dynamique (jalon 2 de l'éditeur) — `SceneConfig`/`LayerConfig`/`ComponentMount` étendus (pas remplacés) : `placement` par composant (pas par couche), bibliothèque de composants étoffée (Box/Divider/TextLabel/TextList/PollBar/Badge/Image), binding déclaratif, `SceneId` ouvert. 6 sessions atomiques, spec `docs/specs/scene-definition-v2.md`. Pivot demandé par l'owner (2026-07-04) : composer/modifier les composants d'une scène + créer/modifier/supprimer des scènes, modèle Figma. Session 3/6 : `TextList` (fin, starting) + `Box` (7 couches) migrés — **vérification visuelle en attente** (owner). | 🔄 en cours (3/6, partielle) |
