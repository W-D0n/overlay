...
| S6 | Contrôle programmatique d'OBS (créer/piloter scènes OBS depuis le panneau S7) — **priorisé par l'owner (2026-07-03)**, voir `docs/inbox.md` §Contrôle OBS centralisé. 4 sessions atomiques, spec `docs/specs/obs-scene-control.md`. Décision owner (2026-07-06) : routes dans `relay/server.js` (seul process tournant pendant le live), pas un process séparé — évite de dupliquer connexion/auth/reconnexion OBS déjà écrites. Session 1/4 **terminée** (2026-07-06) : `GET /obs/list-scenes`, `POST /obs/create-scene`, `POST /obs/set-current-scene`, réutilisent `sendObsRequest` (déjà écrit pour `/refresh-source`). Vérifié avec un faux serveur OBS WS v5 (pas de test unitaire — convention existante des serveurs de dev, effets vérifiés manuellement). Session 4/4 (vérification contre une vraie instance OBS) bloquée : `OBS_WS_URL` pointe une IP LAN injoignable depuis cet environnement. | 🔶 en cours (1/4) |
