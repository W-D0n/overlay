...
| `offsetX` / `offsetY` | 0 à 400 px depuis le coin choisi | `48` |
...
Un **coin** plutôt que des coordonnées libres : le canvas peut changer de taille (1920×1080 hier,
2560×1440 aujourd'hui) et un ancrage en pixels absolus depuis un coin reste correct dans les deux
cas, contrairement à une position absolue.
...
| Fichier | Rôle | Nature |
|---|---|---|
| `branding-format.js` | Normalisation + validation du bloc, styles de position par coin | **Pur, testé** |
...
| `dev/background-tuner.html` + contrôleur | Section « Branding » + case « Afficher sur ce preset » | Existants, étendus |
...
1. `normalizeBranding` : absent, `null`, champs inconnus, tailles hors bornes, `lines` trop longues
   ou trop nombreuses → défauts et bornes, jamais d'exception.
2. `validateBranding` : coin inconnu, `lines` non tableau, `opacity` hors [0,1] → erreurs listées.
3. Un fichier d'état sans `branding` reste valide et n'affiche rien.
4. Chaque coin produit les bonnes propriétés CSS (`top`/`bottom` et `left`/`right`, jamais les
   quatre).
...
3. **Tuner** — section « Branding » (contenu, coin, décalages, tailles, couleur, opacité) + case
...
- **LAC-01** — Le texte n'est pas mis à l'échelle selon la taille du canvas : à 2560×1440, une
  ...
  les tailles restent explicites, à ajuster une fois.
